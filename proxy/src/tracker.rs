        ...
        if let MongoMessage::Query(m) = msg {
            if let Some(op) = m.query.get_str("op") {
                if op == "isMaster" || op == "ismaster" {
                    // The first isMaster message from client contains the app name and possibly
                    // also the username.
                    let mut count_as_connection = false;

                    if let Some(app_name) = m.query.get_str("app_name") {
                        if self.client_application.is_empty() {
                            self.client_application = app_name.to_owned();
                            count_as_connection = true;
                        }
                    }
                    if let Some(username) = m.query.get_str("username") {
                        if self.client_username.is_empty() {
                            self.client_username = username.to_owned();
                            count_as_connection = true;
                        }
                    }
                    if count_as_connection {
